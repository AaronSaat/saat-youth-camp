...
class EvaluasiScreen4 extends StatefulWidget {
  const EvaluasiScreen4({super.key});

  @override
  State<EvaluasiScreen4> createState() => _EvaluasiScreen4State();
}

class _EvaluasiScreen4State extends State<EvaluasiScreen4> {
  ...
  Future<void> _loadSavedProgress() async {
    ...
    setState(() {
      _sliderValue1 = prefs.getDouble('kkr1_slider1') ?? 3;
      _sliderValue2 = prefs.getDouble('kkr1_slider2') ?? 3;
      answerDrama = prefs.getString('kkr1_answer_drama');
      _komentarDramaController.text = prefs.getString('kkr1_komentar_drama') ?? '';
      _saranIbadahController.text = prefs.getString('kkr1_saran_ibadah') ?? '';
    });
  }

  Future<void> _saveProgress() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setDouble('kkr1_slider1', _sliderValue1);
    await prefs.setDouble('kkr1_slider2', _sliderValue2);
    await prefs.setString('kkr1_answer_drama', answerDrama ?? '');
    await prefs.setString('kkr1_komentar_drama', _komentarDramaController.text);
    await prefs.setString('kkr1_saran_ibadah', _saranIbadahController.text);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Evaluasi KKR 1')),
                        ...
                        isLoading
                            ...
                            : () async {
                              ...
                              if (mounted) {
                                ...
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(builder: (context) => const ReviewEvaluasi4Screen()),
                                );
                              }
                            },
                    style: ElevatedButton.styleFrom(
