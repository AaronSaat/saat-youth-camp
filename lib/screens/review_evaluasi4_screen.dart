...
class ReviewEvaluasi4Screen extends StatefulWidget {
  const ReviewEvaluasi4Screen({super.key});

  @override
  State<ReviewEvaluasi4Screen> createState() => _ReviewEvaluasi4ScreenState();
}

class _ReviewEvaluasi4ScreenState extends State<ReviewEvaluasi4Screen> {
  ...
  Future<void> _loadDataFromPrefs() async {
    ...
    setState(() {
      _slider1Value = prefs.getDouble('kkr1_slider1') ?? 0;
      _slider2Value = prefs.getDouble('kkr1_slider2') ?? 0;
      answer1 = prefs.getString('kkr1_answer_drama') ?? '';
      saran = prefs.getString('kkr1_komentar_drama') ?? '';
      komentar = prefs.getString('kkr1_saran_ibadah') ?? '';
      isLoading = false;
  ...
  Future<void> _handleFinalSubmit() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('evaluasi4_status', 'completed');
    await prefs.setDouble('kkr1_slider1', _slider1Value);
    await prefs.setDouble('kkr1_slider2', _slider2Value);
    await prefs.setString('kkr1_answer_drama', answer1 ?? '');
    await prefs.setString('kkr1_komentar_drama', saran);
    await prefs.setString('kkr1_saran_ibadah', komentar);
    ...
    showDialog(
      ...
      builder:
          (_) => AlertDialog(
            ...
            content: const Text('Evaluasi KKR 1 Anda telah dicatat.'),
            actions: [
              TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Review Evaluasi KKR 1')),
