...
class ListDevotionScreen extends StatefulWidget {
  const ListDevotionScreen({super.key});

  @override
  State<ListDevotionScreen> createState() => _ListDevotionScreenState();
}

class _ListDevotionScreenState extends State<ListDevotionScreen> {
  ...
  Future<void> loadDevotion({bool forceRefresh = false}) async {
    if (!mounted) return;
    setState(() {
      _isLoading = true;
    });
    try {
      final prefs = await SharedPreferences.getInstance();
      const tutorialKey = 'devotion_list_with_meta';
      ...
      if (!forceRefresh) {
        ...
        if (cachedTutorial != null) {
          ...
          final devotionWithMeta = decoded.cast<Map<String, dynamic>>();
          setState(() {
            _devotionList = devotionWithMeta;
            _isLoading = false;
          });
          print('[PREF_API] dari shared pref Devotion List');
          return;
        }
      }
      ...
      final devotionWithMeta = await Future.wait(
        devotionList.map((devotion) async {
          try {
            ...
            return {...devotion, 'meta': meta};
          } catch (e) {
            return {...devotion, 'meta': {}};
          }
        }),
      );
      // Simpan ke shared pref
      await prefs.setString(tutorialKey, jsonEncode(devotionWithMeta));
      if (!mounted) return;
      setState(() {
        _devotionList = devotionWithMeta;
        _isLoading = false;
        print('[PREF_API] dari API Devotion List');
      });
    } catch (e) {
      print('❌ Gagal memuat devotion: $e');
      if (!mounted) return;
      setState(() {
        _isLoading = false;
      });
          ...
          children: [
            ...
            SafeArea(
              child: RefreshIndicator(
                onRefresh: () => loadDevotion(forceRefresh: true),
